...
function getPageContent(versionMatch, includePrerelease) {
    ...
    function getHighestMatchingAlphaVersion(versions, pattern) {
        var regexPattern = pattern.replace('.', '\\.').replace('x', '\\d+');
        var regex = new RegExp("^".concat(regexPattern));
        var filteredVersions = versions.filter(function (version) { return regex.test(version); });
        if (filteredVersions.length === 0) {
            return null;
        }
        var sortedVersions = filteredVersions.sort(semver.rcompare);
