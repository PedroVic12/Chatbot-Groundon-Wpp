...
class GroundonView extends Groundon {
	...
	async sendLinkCardapioDigital(message, _LINK) {
		...
		const PROCESSING_MESSAGE_DELAY = 7000; // 7 seconds
		...
		const sendLink = async (attempt = 1) => {
			...
			// Envia mensagem de processamento se passar de 7 segundos
			const processingTimer = setTimeout(() => {
				if (!linkSent) {
					this.enviarMensagem(message, `Processando... por favor aguarde.`);
					processingMessageSent = true;
				}
			}, PROCESSING_MESSAGE_DELAY);
